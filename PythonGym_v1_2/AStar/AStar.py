...
class AStar:
    ...
    def GetPlan(self):
        ...
        while self.open:
            ...
            # Expandir sucesores
            for succ in self.problem.GetSucessors(current):
                # Ignorar si ya procesado
                if succ in self.processed:
                    continue

                # Calcular coste tentativo
                tentativeG = current.G() + self.problem.GetGCost(succ)

                # Chequear en frontera
                existing = self.GetSucesorInOpen(succ)
                if existing is None:
                    # Nuevo nodo en frontera
                    self._ConfigureNode(succ, current, tentativeG)
                    self.InsertNode(succ)
                elif tentativeG < existing.G():
                    # Mejorar camino existente
                    self._ConfigureNode(existing, current, tentativeG)
                    self.open.remove(existing)
                    self.InsertNode(existing)
...
class AStar:
    ...
    #nos dice si un sucesor esta en abierta. Si esta es que ya ha sido expandido y tendra un coste, comprobar que le nuevo camino no es mas eficiente
    #En caso de serlos, _ConfigureNode para setearle el nuevo padre y el nuevo G, asi como su heuristica
    def GetSucesorInOpen(self,sucesor):
        i = 0
        found = None
        while found == None and i < len(self.open):
